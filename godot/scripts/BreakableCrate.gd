...
## A destructible 3D crate prop. Plugs into the existing melee/skill hit
...
const _MODEL_PATH := "res://Assets/Crate/Meshes/crate.glb"
# Crate exported at 0.85× Blender scale → ~0.646m cube.
# Box collider slightly inset from true edge so it doesn't clip wall geometry.
const _HALF_SIZE := 0.30   # half-width/depth in world space (m)
const _HEIGHT    := 0.646  # full height (m) — sets collider + shadow radius
...
func _ready() -> void:
	...
	shape.size = Vector3(_HALF_SIZE * 2.0, _HEIGHT, _HALF_SIZE * 2.0)
	col.shape = shape
	...
	if sf != null and sf.has_method("make_shadow"):
		var shadow: Node3D = sf.make_shadow(0.42)
...
## works unmodified against a crate with no special-casing.
...
func _spawn_hit_sparks() -> void:
	...
	if pf == null or not pf.has_method("hit_sparks"):
		return
	var p: GPUParticles3D = pf.hit_sparks(Color(0.60, 0.40, 0.18))
	add_child(p)
...
func _break() -> void:
	...
	if pf != null and pf.has_method("hit_sparks"):
		var burst: GPUParticles3D = pf.hit_sparks(Color(0.60, 0.40, 0.18))
		burst.amount = 22
		burst.lifetime = 0.6
		burst.top_level = true
		add_child(burst)
		burst.global_position = global_position + Vector3(0.0, 0.35, 0.0)
		burst.emitting = true
