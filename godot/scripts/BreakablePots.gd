...
## A destructible 3D terracotta pot cluster prop. Plugs into the existing
## melee/skill hit detection by joining group "enemy" — Player._resolve_melee_hit,
...
const _MODEL_PATH := "res://Assets/Pots/Meshes/pots.glb"
# Cluster exported at 0.72× Blender scale → ~0.547m tall, ~0.806m wide footprint.
# Cylinder collider fits the roughly-round cluster better than a box.
const _RADIUS := 0.30   # fits the cluster footprint (0.806m wide / 2 = 0.403, inset slightly)
const _HEIGHT := 0.547  # full exported height (m)

@export var hp := 10.0
var _broken := false

# Pots shatter easily (classic "smash for coins" trope) — slightly higher coin
# chance than barrels; mirrors Enemy._drop_loot's GroundItem spawn pattern.
const _LOOT_POOL: Array    = ["gold", "hpot", "mpot"]
const _LOOT_WEIGHTS: Array = [0.40,   0.08,   0.03]   # remaining 49% = nothing
...
func _ready() -> void:
	...
	if sf != null and sf.has_method("make_shadow"):
		var shadow: Node3D = sf.make_shadow(0.38)
...
## works unmodified against a pot cluster with no special-casing.
...
func _spawn_hit_sparks() -> void:
	...
	if pf == null or not pf.has_method("hit_sparks"):
		return
	var p: GPUParticles3D = pf.hit_sparks(Color(0.6, 0.32, 0.18))
	add_child(p)
...
func _break() -> void:
	...
	if pf != null and pf.has_method("hit_sparks"):
		var burst: GPUParticles3D = pf.hit_sparks(Color(0.6, 0.32, 0.18))
		burst.amount = 22
		burst.lifetime = 0.6
		burst.top_level = true
		add_child(burst)
		burst.global_position = global_position + Vector3(0.0, 0.25, 0.0)
		burst.emitting = true
